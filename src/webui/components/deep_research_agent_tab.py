...
def _is_safe_subpath(candidate_path: str, root_path: str) -> bool:
    """Return True when candidate_path stays within root_path."""
...
async def run_deep_research(webui_manager: WebuiManager, components: Dict[Component, Any]) -> AsyncGenerator[
    ...
    if not _is_safe_subpath(normalized_base_save_dir, safe_root_dir):
...
async def stop_deep_research(webui_manager: WebuiManager) -> Dict[Component, Any]:
    ...
    if agent and task and not task.done():
        ...
        final_update = {
            ...
            webui_manager.get_component_by_id("deep_research_agent.parallel_num"): gr.update(interactive=True),
